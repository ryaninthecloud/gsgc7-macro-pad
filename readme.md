...
A[User] -->|Inputs number| B(GSGC7)

B --> C(Arduino)

C -->|Number dialled| D[Python Script]
...
```
 ...
 - `arduino/gsgc7-connection.ino` - a simple Arduino INO sketch that listens for and broadcasts messages to and from the GSGC7, it is merely a conduit for passing the number dialled to the Python script. Nothing fancy.
 - `gsgc7interface.py` - where the bulk of the code is. This class detects the operating system of the host, loads the catalogue and then listens for messages from the GSGC7. Once the interface receives a message it processes it, looking for any matching catalogue entries and executes the commands defined in the catalogue. The interface finishes its interactions with the GSGC7 by sending simple responses that illuminate the device's LEDs to indicate success or failure.
 - `catalogue.py` - this is probably the most interesting place. This is where the automations or macro-actions are defined. Actions can either be defined separately in functions from other .py files, defined in the file or just defined using Lambda functions against the key in the dictionary.

### What's next

 - Refining the automations in the catalogue
 - Ensuring MacOS compatibility
 - Further tinkering with the capabilities of the GSGC7 -- namely, right now, investigating this strange behaviour I've observed where I can get the display to display a text message if I continuously batter it with messages.
 - Seeing if I can reduce time to dial in some way
 - Turning the Python interface into a background process/service!
